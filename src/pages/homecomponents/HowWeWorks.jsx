    ...
    <div className="w-[90%] h-[600px] flex flex-col items-center justify-center px-10 py-5 mb-20 ">
      ...
      >
        {WorksData.map((value, index) => {
          return (
            <div key={index} className="w-[25%] h-80 flex flex-col justify-center items-center p-3">
              <img src={value.img} alt={value.title} className="w-60 h-60" />
              <h3 className="text-center text-[#002948] text-2xl font-semibold pt-2 font-[none]">{value.title}</h3>
              <p className="text-center text-lg text-gray-600 pt-2">{value.info}</p>
            </div>
          )
        })}
      </div>
