...
function HomeRewards() {
  ...
  const increaseValue = (currentValue, setValue, target, step, intervalTime) => {
  ...
  const rewards = [
    { Icon: 'healthicons:doctor-male', number: `${satisfiedClients}+`, title: 'Qualified Doctors' },
    { Icon: 'dashicons:businesswoman', number: `${currentNumber}+`, title: 'Satisfied Clients' },
    { Icon: 'line-md:star-pulsating-loop', number: `${awardWinning}+`, title: 'Award Winning' },
    { Icon: 'streamline:customer-support-1-solid', number: `${clientSupport}/7`, title: 'Client Support' },
  ];
    ...
    <div className="w-full h-80 mb-20 flex justify-center items-center ">
      <div className="w-[82%] h-72 bg-[#045e74] rounded-xl px-4 flex gap-3 justify-between items-center">
        {rewards.map((reward, index) => {
          return (
            <div key={index} className="w-[20%] h-64 flex flex-col justify-center items-center ">
              <Icon icon={reward.Icon} width={90} height={90} style={{ color: '#fff' }} />
              <h1 className='text-center text-white text-6xl -mt-3 font-[Katibeh]'>{reward.number}</h1>
              <p className='text-center text-white text-4xl font-[Katibeh]'>{reward.title}</p>
            </div>
          );
        })}
      </div>
